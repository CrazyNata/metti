...
Защищённая Edge Function `metti-stylist` развёрнута с обязательной проверкой JWT. Она сама загружает гардероб и профиль пользователя, а затем передаёт в AI только эти данные вместе с запросом и погодой. Для включения ответа модели добавьте секрет `OPENAI_API_KEY` в Supabase Dashboard → Edge Functions → Secrets (ключ никогда не хранится в приложении). Не добавляйте этот секрет в Git.
